...
{
	return Render(deviceContext, m_x, m_y);
}
...
bool BitmapClass::Render(ID3D11DeviceContext* deviceContext, int positionX, int positionY)
{
	...
	result = UpdateBuffers(deviceContext, positionX, positionY, m_width, m_height);
	if(!result)
	{
		return false;
	}
...
void BitmapClass::SetDimensions(int width, int height)
{
	m_width = width;
	m_height = height;
}
...
int BitmapClass::GetX()
{
	return m_x;
}

int BitmapClass::GetY()
{
	return m_y;
}
...
bool BitmapClass::UpdateBuffers(ID3D11DeviceContext* deviceContext, int positionX, int positionY, int width, int height)
{
	...
	if((positionX == m_previousPosX) && (positionY == m_previousPosY) 
		&& (width == m_previousWidth) && (height == m_previousHeight))
	{
		return true;
	}
